    ...
    @foreach ($data as $item)
      <div class="w-full bg-white border border-gray-200 rounded-lg shadow-lg dark:bg-gray-800 dark:border-gray-700 relative">
        <a href="{{ route('bmn.show', [$ruangan, $item->id]) }}">
          @if ($item->foto)
            <img class="w-full rounded-t-lg h-40 object-cover" src="{{ asset('storage/' . $item->foto) }}" alt="Foto Barang">
          @else
            <img class="w-full rounded-t-lg h-40 object-cover opacity-50" src="{{ asset('img/no-image.png') }}" alt="No Image">
          @endif
        </a>
